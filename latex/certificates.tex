\documentclass[10pt,a4paper,sans,colorlinks]{moderncv}
\title{Certificates}
\input{shared.tex}
\setLargeGeometry
\setParagraphReducedSpacing
\usepackage{graphbox}
\usepackage{tabularx}
\usepackage{graphicx}
\usepackage{pgffor}
\usepackage{pgfkeys, pgfcalendar}
\usepackage{float}

% modern cv title contents
\NewCommandCopy{\oldsection}{\section}
\renewcommand{\section}[1]{
  \oldsection{#1}
  \addcontentsline{toc}{part}{#1}
}
\makeatletter
\newcommand\@pnumwidth{1.55em}
\newcommand\@tocrmarg{2.55em}
\newcommand\@dotsep{8}
\setcounter{tocdepth}{0}
\newcommand\tableofcontents{%
  \addtocontents{toc}{\protect\setcounter{tocdepth}{0}}
  \newbox{\makecvtitlenamebox}
  \savebox{\makecvtitlenamebox}{\namestyle{\@firstname\ \@lastname}}
  \@starttoc{toc}%
}
\usepackage{titletoc}
\titlecontents*{section}[0pt]{}{}{\textbullet}{\ \thecontentspage}[\\][]
\titlecontents{part}[0pt]{}{}{}{\leaders\hbox{\normalfont$\m@th\mkern \@dotsep mu\hbox{.}\mkern \@dotsep mu$}\hfill\normalsize\thecontentspage}[]
\makeatother
% modern cv title contents

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% document
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{document}
\setHyperrefBlueLinks
\makecvtitle
\tableofcontents

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% B.Sc. diploma
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
\section{B.Sc. diploma}
The next Figure presents my B.Sc. diploma in Portuguese.
It says a B.Sc. in Computer Science degree from the Federal University of Paraiba, Brazil.

\begin{figure}
  \centering
  \includegraphics[align=t,width=\textwidth, keepaspectratio=true]{../static/certificates/bsc-diploma.pdf}
\end{figure}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% B.Sc. grades
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
\section{B.Sc. grades}

The next Figure presents the first part of the grades of my B.Sc. in Portuguese.
It informs that is been done with a general score (CRA in Portuguese) of \textbf{7.72}.
It includes three Physics and three Differential Calculus courses.
From computer science fundaments, it includes two Data Structures courses, Logic, and  Design and Analyses of Algorithms. In particular, I archive the max degree 10.
\vspace{2em}
\begin{figure}
  \centering
  \includegraphics[align=t,width=\textwidth,height=0.75\textheight, keepaspectratio=true, trim=0cm 0cm 0cm 2cm]{../static/certificates/bsc-grades.pdf}
\end{figure}

\newpage

The next Figure presents the second part of the grades of my B.Sc. in Portuguese.
The courses include Data Bases, Software Engineering, Computer Networks, and Compilers.
Moreover, in special topics, I coursed Psychology, Multimedia Systems, and Economics. It also shows a total 2895  of hours.
\vspace{2em}
\begin{figure}
  \centering
  \includegraphics[align=t,width=\textwidth,height=0.75\textheight, keepaspectratio=true,page=2, trim=0cm 0cm 0cm 2cm]{../static/certificates/bsc-grades.pdf}
\end{figure}

\newpage

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% MSc diploma
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{MSc diploma}

The next Figure presents my MSc diploma in Portuguese.
It informs an MSc degree in Informatics also from the Federal University of Paraíba, Brazil.

\begin{figure}
  \centering
  \includegraphics[align=t,width=\textwidth, keepaspectratio=true]{../static/certificates/msc-diploma.pdf}
\end{figure}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% MSc grades
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
\section{MSc grades}
The next Figure presents the first part of the grades of my MSc in Portuguese.
It informs that is been done with a general score (CRA in Portuguese) of \textbf{8.12}.
It includes the courses Analyses of Algorithms, Computer Architecture, and Computing Theory.
In special topics, I coursed Distributed Systems and Multimedia Systems.
\vspace{1em}
\begin{figure}
  \centering
  \includegraphics[align=t,width=\textwidth,height=0.75\textheight, keepaspectratio=true, trim=0cm 0cm 0cm 2cm]{../static/certificates/msc-grades.pdf}
\end{figure}

\newpage
The next Figure presents the second part of the grades of my B.Sc. in Portuguese.
It informs the Master Thesis named \href{https://repositorio.ufpb.br/jspui/handle/tede/6087}{"A Solution for Running Ginga-NCL Applications Using Second Screen on BroadBandTV Systems"}.
\vspace{1em}
\begin{figure}
  \centering
  \includegraphics[align=t,width=\textwidth,height=0.75\textheight, keepaspectratio=true, page=2, trim=0cm 0cm 0cm 2cm]{../static/certificates/msc-grades.pdf}
\end{figure}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Ph.D. diploma
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
\section{Ph.D. diploma}

The next Figure presents my Ph.D. diploma in Portuguese.
It says: "The dean of Pontifical Catholically University of Rio de Janeiro, in his attributions and given that were be fulfilled all legal requirements, as well the university requirements, grants to: Alan Livio Vasconcelos Guedes, the degree of Doctor in Science - Informatics with all the prerogatives, rights, and honors that are given"
The bottom part contains signatures from both the director of the "Admission and Registry" department and the dean.

\begin{figure}
  \centering
  \includegraphics[align=t,width=\textwidth,height=0.75\textheight, keepaspectratio=true, trim={6cm 0 3cm 0},clip]{../static/certificates/phd-diploma.pdf}
\end{figure}

\newpage

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Ph.D. grades
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Ph.D. grades}

The next Figure presents the first page of grades from my Ph.D. in Portuguese. Among others, it includes courses in Computer Graphics, Mobile Computing, and Multimedia Systems.

\begin{figure}
  \centering
  \includegraphics[align=t,width=\textwidth,height=0.75\textheight, keepaspectratio=true]{../static/certificates/phd-grades.pdf}
\end{figure}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Ph.D. grades second
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
The next Figure presents the second page of grades from my Ph.D. in Portuguese. In particular, it shows the thesis proposal exam as AP (approved).

\begin{figure}
  \centering
  \includegraphics[align=t,page=2,width=\textwidth,height=0.75\textheight, keepaspectratio=true]{../static/certificates/phd-grades.pdf}
\end{figure}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% UCL certificates
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
\section{UCL research staff certificates}
The next Figures present UCL certificates in research staff training topics of Data protection, People Management, Diversity, and Research Funding.

\begin{figure}
  \centering \includegraphics[align=t,width=\textwidth,height=0.33\textheight, keepaspectratio=true]{../static/certificates/People_Management_Essentials.pdf}
  \centering \includegraphics[align=t,width=\textwidth,height=0.33\textheight, keepaspectratio=true]{../static/certificates/EDI.pdf}
\end{figure}
\begin{figure}
  \centering \includegraphics[align=t,width=\textwidth,height=0.33\textheight, keepaspectratio=true]{../static/certificates/GDPR.pdf}
  \centering \includegraphics[align=t,width=\textwidth,height=0.33\textheight, keepaspectratio=true]{../static/certificates/Data_Protection_and_Freedom_information.pdf}
  \includegraphics[align=t,width=\textwidth,height=0.33\textheight, keepaspectratio=true]{../static/certificates/Fundamentals_in_Research_Funding.pdf}
\end{figure}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Nature certificates
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
\section{Nature MasterClass certificates}
The next Figures present Nature MasterClass certificates in research topics of Peer Review and Narrative Tools.

\begin{figure}
  \centering
  \includegraphics[align=t,width=\textwidth,height=0.4\textheight, keepaspectratio=true]{../static/certificates/Focus_on_Peer_Review.pdf}
  \includegraphics[align=t,width=\textwidth,height=0.4\textheight, keepaspectratio=true]{../static/certificates/Narrative_Tools_for_Researchers.pdf}

\end{figure}

\end{document}